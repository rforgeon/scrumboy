...
export function normalizeAgendaStartOfDay(value: unknown): string {
  const raw = typeof value === 'string' ? value.trim() : '';
  const match = TIME_PATTERN.exec(raw);
  if (!match) return AGENDA_START_OF_DAY_DEFAULT;
  return `${match[1].padStart(2, '0')}:${match[2]}`;
}

export function agendaStartOfDayMinutes(value?: string): number {
...
function writeLocalPreference(hhmm: string, ownerUserId: number | null): void {
...
export function getAgendaStartOfDayPreference(): string {
  try {
    const user = getUser();
...
export function setAgendaStartOfDayPreference(hhmm: string): void {
...
export async function saveAgendaStartOfDayPreference(hhmm: string): Promise<void> {
  const next = normalizeAgendaStartOfDay(hhmm);
  const user = getUser();
...
export function hydrateAgendaStartOfDayFromServer(value: unknown): void {
...
export async function loadAgendaStartOfDayPreferenceFromServer(
 ...
 */
export function onAgendaStartOfDayAuthUserChanged(userId: number | null): void {
