...
export type AgendaNowLineStyle = typeof AGENDA_NOW_LINE_SUBTLE | typeof AGENDA_NOW_LINE_PROMINENT;

let saveEpoch = 0;
let ownerGeneration = 0;
let inFlightSaves = 0;
let lastSuccessEpoch = 0;
let lastConfirmed: AgendaNowLineStyle = AGENDA_NOW_LINE_DEFAULT;

export function normalizeAgendaNowLine(value: unknown): AgendaNowLineStyle {
...
export function isAgendaNowLineProminent(value?: string): boolean {
...
function writeLocalPreference(style: AgendaNowLineStyle, ownerUserId: number | null): void {
...
export function getAgendaNowLinePreference(): AgendaNowLineStyle {
  try {
    const user = getUser();
...
export function setAgendaNowLinePreference(style: unknown): void {
...
export async function saveAgendaNowLinePreference(style: unknown): Promise<void> {
  const next = normalizeAgendaNowLine(style);
  const user = getUser();
...
export function hydrateAgendaNowLineFromServer(value: unknown): void {
...
export async function loadAgendaNowLinePreferenceFromServer(
...
export function onAgendaNowLineAuthUserChanged(userId: number | null): void {
